...
export const CheckoutForm = () => {
    ...
    const handleSubmit = async (ev: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!paymentIntentClientSecret || !card)
            return null

        ev.preventDefault();
        setProcessing(true);
        
        const payload = await stripe.confirmCardPayment(paymentIntentClientSecret, {
            payment_method: { card }
        });
                ...
                <>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 5 }}>
                        <Button
                            ...
                            variant="contained"
                            size='small'
                            startIcon={processing ? <ClipLoader color='white' size={10} speedMultiplier={0.5} /> : <PaymentIcon />}
                        >
