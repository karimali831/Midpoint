...
export function* getUser() {
    try{
        ...
        const user: IUser = yield call(userApi.getUserByFirebaseUid, firebaseUid);
...
export function* firebaseAuthenticated(action: PayloadAction<IFirebaseUser>) {
    try {
        ...
        if (user.createdInstanceId) {
            const response : EC2Response = yield call(ec2InstanceApi.get, user.createdInstanceId, user.id);
